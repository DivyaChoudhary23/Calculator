...
class Calc():
        ...
        if self.input_value:
            ...
            if secondnum == ".":
                if secondnum in firstnum:
                    return
            self.current = secondnum + firstnum
    ...
    #function for add, sub,mul,div,mod
...
txtDisplay = Entry(calc, font=('arial','20','bold'), bg="white", bd=25, width= 29, justify=CENTER)
txtDisplay.grid( column=0, columnspan=4, pady=1 , row=0)
...
btnSqrt = Button(calc, text="√ ", width=6, height=2, font=('arial', 20, 'bold'),
                 bd=4, bg="aquamarine", command=added_value.sqrt).grid(row=1, column=2, pady=1)
...
btnMult = Button(calc, text="x", width=6, height=2, font=('arial', 20, 'bold'), bd=4,
                 bg="aquamarine", command=lambda: added_value.operation("mul")).grid(row=1, column=3, pady=1)

# the '÷' Button
btnDiv = Button(calc, text="÷", width=6, height=2, font=('arial', 20, 'bold'), bd=4,
                bg="aquamarine", command=lambda: added_value.operation("div")).grid(row=2, column=3, pady=1)
